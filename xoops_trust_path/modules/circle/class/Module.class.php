...
define('CIRCLE_FRAME_PERFORM_SUCCESS', 1);
define('CIRCLE_FRAME_PERFORM_FAIL', 2);
define('CIRCLE_FRAME_INIT_SUCCESS', 3);

define('CIRCLE_FRAME_VIEW_NONE','none');
define('CIRCLE_FRAME_VIEW_SUCCESS','success');
define('CIRCLE_FRAME_VIEW_ERROR','error');
define('CIRCLE_FRAME_VIEW_INDEX','index');
define('CIRCLE_FRAME_VIEW_INPUT','input');
define('CIRCLE_FRAME_VIEW_PREVIEW','preview');
define('CIRCLE_FRAME_VIEW_CANCEL','cancel');

/**
 * Circle_Module
**/
class Circle_Module extends Legacy_ModuleAdapter
{
	public /*** string ***/ $mActionName = null;

	public /*** Circle_AbstractAction ***/ $mAction = null;

	public /*** bool ***/ $mAdminFlag = false;

	public /*** Circle_AssetManager ***/ $mAssetManager = null;
	...
	protected /*** Enum[] ***/ $_mAllowViewNames = array(
		CIRCLE_FRAME_VIEW_NONE,
		CIRCLE_FRAME_VIEW_SUCCESS,
		CIRCLE_FRAME_VIEW_ERROR,
		CIRCLE_FRAME_VIEW_INDEX,
		CIRCLE_FRAME_VIEW_INPUT,
		CIRCLE_FRAME_VIEW_PREVIEW,
		CIRCLE_FRAME_VIEW_CANCEL
	);
	...
	**/
	public function startup()
	{
		...
		XCube_DelegateUtils::call('Module.circle.Global.Event.GetAssetManager',new XCube_Ref($this->mAssetManager),$this->mXoopsModule->get('dirname'));
	...
	**/
	private function _getDefaultActionName()
	{
		...
		$dataname = isset($dataname) ? $dataname : 'People';
	...
	**/
	public function getRenderSystemName()
	{
		...
		if(!defined('CIRCLE_ADMIN_RENDER_REGISTED'))
		{
			define('CIRCLE_ADMIN_RENDER_REGISTED',true);
			$root =& XCube_Root::getSingleton();
			$root->overrideSiteConfig(
				array(
					'RenderSystems' => array(
						'Circle_AdminRenderSystem' => 'Circle_AdminRenderSystem'
					),
					'Circle_AdminRenderSystem' => array(
						'root' => CIRCLE_TRUST_PATH,
						'path' => '/admin/class/CircleAdminRenderSystem.class.php',
						'class' => 'Circle_AdminRenderSystem'
					)
				)
			);
		}
	
		return 'Circle_AdminRenderSystem';
	}
	...
	private function _createAction()
	{
		...
		switch(true)
		{
			case file_exists(
				...
				break;
			case file_exists(
				$path = CIRCLE_TRUST_PATH . '/' . $fileName
			):
				break;
			default:
				return false;
		...
		$className = 'Circle_' . ($this->mAdminFlag ? 'Admin_' : '')
			. ucfirst($this->mActionName) . 'Action';
		if(class_exists($className))
		{
			$this->mAction = new $className();
		}
		if(!$this->mAction instanceof Circle_AbstractAction)
		{
			return false;
		}
	...
	**/
	...
	{
		/**
		 * Module.circle.Global.Event.Exception.ActionNotFound
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.circle.Global.Event.Exception.ActionNotFound',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.circle.Global.Event.Exception.Preparation
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.circle.Global.Event.Exception.Preparation',$this->mAssetManager->mDirname);
		/**
	...
	**/
	...
	{
		/**
		 * Module.circle.Global.Event.Exception.Permission
		 * 
		 * @param	string	$dirname
		 * 
		 * @return	void
		**/
		XCube_DelegateUtils::call('Module.circle.Global.Event.Exception.Permission',$this->mAssetManager->mDirname);
		/**
		...
		**/
		...
		$root =& XCube_Root::getSingleton();
		$root->mController->executeRedirect(XOOPS_URL, 1, _MD_CIRCLE_ERROR_PERMISSION);
	}
}
